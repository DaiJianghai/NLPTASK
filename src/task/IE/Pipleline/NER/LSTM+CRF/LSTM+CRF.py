...
class Mymodel(nn.Module):
    def __init__(self,corpus_num,embedding_num,hidden_num,class_num,bi=True, w2i=None):
        super().__init__()
    ...
    def forward(self,batch_data,batch_tag=None):
        ...
        emission = self.classifier(out) # [batch, seq, num_tag]
        # self.pre = torch.argmax(pre, dim=-1).reshape(-1)
        self.pre = torch.tensor(self.crf.decode(emission)).reshape(-1)
        if batch_tag is not None:
            mask = (batch_data != self.w2i['<PAD>']).byte()
            loss = -self.crf(emission, batch_tag, reduction="mean")
            # emission_loss = self.cross_loss(emission.reshape(-1, emission.shape[-1]), batch_tag.reshape(-1))
            # loss = emission_loss + neg_log_loss
            return loss
...
if __name__ == "__main__":
    ...
    model = Mymodel(corpus_num,embedding_num,hidden_num,class_num,bi, word_2_index)
