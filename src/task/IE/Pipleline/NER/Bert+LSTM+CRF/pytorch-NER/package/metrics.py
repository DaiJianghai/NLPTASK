...
from utils import decode_bio_tags
...
class Score:
    ...
    def update(self, input, target):
        ...
        for y_pred, y_target in zip(input, target):
            ...
            self.origins.extend(y_target)  # [['book', 1, 2], ['name', 2, 3], ['name', 17, 19], ['name', 23, 25]]
            self.founds.extend(y_pred)      # [['book', 1, 2], ['name', 2, 3], ['name', 17, 19], ['name', 23, 25]]   
            self.rights.extend([y for y in y_pred if y in y_target])

if __name__ == '__main__':
    x = [['book', 1, 2], ['name', 2, 3], ['name', 16, 19], ['name', 23, 25]]
    y = Counter([t for t, _, _ in x])
    print(y)
