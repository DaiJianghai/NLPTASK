...
class CLUEDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        """ # todo 每个batch内的处理，这样的好处是可以根据当前batch的长度来动态调整长度，一个batch内的数据维度一样就行了，所以不用设置max_len
        # todo 可以不像这个代码中这样处理，直接encode处理也行，只要保证结果一样就行

        Returns:
            _type_: {
                "input_ids": [batch_size, max_len], 注意word piece 问题
                "attention_mask": [batch_size, max_len], [cls], [sep] 需要attention
                "labels": [batch_size, max_len] 标签, padding 为 "O", spicial token为 "O"
            }
        """
        texts, labels = list(zip(*[[item[0], item[1]] for item in batch]))   
        token = self.tokenizer(list(texts), padding=False, return_offsets_mapping=True) # return_offsets_mapping 表示 每一个tokenzied后的token对应原始文本 的起始和终止位置-1
        ...
        return torch.LongTensor(token['input_ids']), torch.ByteTensor(token['attention_mask']), torch.LongTensor(labels)
